...
switch OPTtype
    case 'OPLS'
        switch OPLSoption
            ...
            case 3 %OSC-> PCR
                [Z,W,Pv,T] = dosc(data,responseNormal,nOrthcomp,1E-3);
                [tcoeffs, tscores, tlatent,ttsquare] = princomp(Z);
            ...
            case 4 %PLS
                ...
                pcr_vector = BETA(2:end);
            case 5 %PCR
                [tcoeffs, tscores, tlatent,ttsquare] = princomp(data);
                [b,bint,r,rint,stats] = regress(responseNormal,[ones(size(data,1),1)  tscores(:,OPTncomp)]);
                pcr_vector = tcoeffs(:,1:OPTncomp)*b(2:end);
        end
        ...
        case 'OPLS'
            switch OPLSoption
                case 1 %all: OSC->PLS->Reg
                    ...
                    if OPLSIncludeOrtho
                        [b_used,bint_used,r_used,rint_used,stats_used] = regress(responseNormal,[ones(size(data,1),1)  data*W_used data*XL_used(:,1:OPTncomp)]);
                        pcr_vector_used = [W_used XL_used(:,1:OPTncomp)]*b_used(2:end);
                    else
                        [b_used,bint_used,r_used,rint_used,stats_used] = regress(responseNormal,[ones(size(data,1),1)  data*XL_used(:,1:OPTncomp)]);
                        pcr_vector_used = [XL_used(:,1:OPTncomp)]*b_used(2:end);
                    end
                case 2 %OSC-> PLS
                    [Z_used,W_used,Pv_used,T_used] = dosc(data_used,responseNormal,nOrthcomp,1E-3);
                    [XL_used,YL,XS,YS,BETA,PCTVAR,MSE,stats_used] = plsregress(Z_used,responseNormal,OPTncomp);
                ...
                case 3 %OSC-> PCR
                    [Z_used,W_used,Pv_used,T_used] = dosc(data_used,responseNormal,nOrthcomp,1E-3);
                    [tcoeffs_used, tscores_used, tlatent_used,ttsquare_used] = princomp(Z_used);
                ...
                case 4 %PLS
                    ...
                    pcr_vector_used = BETA(2:end);
                case 5 %PCR
                    [tcoeffs_used, tscores_used, tlatent_used,ttsquare_used] = princomp(data_used);
                    [b,bint,r,rint,stats] = regress(responseNormal,[ones(size(data,1),1)  tscores_used(:,OPTncomp)]);
                    pcr_vector_used = tcoeffs_used(:,1:OPTncomp)*b(2:end);
            end
        case 'PCA'
            [loading_used, scores_used, latent,tsquare] = princomp(data_used);
